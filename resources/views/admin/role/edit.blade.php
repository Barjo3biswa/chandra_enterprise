...
<div class="row clearfix">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="card">
			...
			<div class="body">
				<form id="form_validation" method="POST" action="{{ route('update-role', Crypt::encrypt($selected_user->model_id)) }}">
					{{ csrf_field() }}
					{!! method_field('PATCH') !!}
							...
							<div class="form-group form-float">
								<div class="form-line">
									<select class="form-control show-tick" name="user_id">
										...
										<option value="{{ $user->id }}" {{ old('user_id',$selected_user->model_id) == $user->id ? 'selected' : '' }}>{{ ucwords($user->first_name.' '.$user->middle_name.' '.$user->last_name) }}</option>
										<?php endforeach; ?>
									</select>
						...
						@endphp
							...
							@foreach($permission->permission_roles as $pp)
								<div class="col-md-3">
									...
									<?php foreach($user_permission_detail as $key=>$val){
										if($val->id == $pp->id) echo "checked";
								    }?>/>
