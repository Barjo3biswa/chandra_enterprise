...
<div class="row clearfix">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="card">
			...
			<div class="body">
				<form id="form_validation" method="POST" action="{{ route('add-new-company.post') }}">
					{{ csrf_field() }}
								...
								<div class="form-line">
									<select class="form-control show-tick" name="state" id="state">
		                               ...
		                               <?php foreach ($states as $state): ?>
					                    <option value="{{ $state->name }}" data-themeid="{{ $state->id }}" {{ old('state') == "$state->id" ? 'selected' : '' }}>{{ ucwords($state->name) }}</option>
					                   <?php endforeach; ?> 
		                            </select>
	                            </div>
	                        </div>
